...
function initiate(server, config) {
    ...
    var stream
    var command = `rpicam-vid --width ${config.video.width} --height ${config.video.height} -t 0 --framerate ${fps} --inline --bitrate ${bitrate} --profile baseline --nopreview -o - | socat - udp-sendto:${config.host}:${config.port_udp},shut-none`
    ...
    server.on("car-conf", async conf => {
        if (conf.buttons.padUp == 1 || conf.buttons.padDown == 1) {
            ...
            command = `rpicam-vid --width ${config.video.width} --height ${config.video.height} -t 0 --framerate ${fps} --inline --bitrate ${bitrate} --profile baseline --nopreview -o - | socat - udp-sendto:${config.host}:${config.port_udp},shut-none`
